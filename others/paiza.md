...
- [paiza問題](#paiza問題)
    ...
    - [C126:宿泊費と交通費](#c126宿泊費と交通費)
    - [C092:工場のベルトコンベア](#c092工場のベルトコンベア)
    - [C100:選曲の方法](#c100選曲の方法)
    - [C071:直角三角形](#c071直角三角形)
- [苦戦したもの](#苦戦したもの)
...
### [C126:宿泊費と交通費](https://paiza.jp/works/challenges/615/page/result)

- ポイントはインターンの間の期間の宿泊料金の合計が新幹線往復より安いかどうか
...
n.times {|i| inturn << gets.split(" ").map(&:to_i)}

# 料金の合計
# 必ず行きと帰りは新幹線を使うので初期値に往復の料金を入れてある
total = a * 2 

# 次のインターン開始日から前のインターン終了日を引いているので、範囲を間違えないように
(0...n-1).each do |i|

    # インターンの間の期間を定義
    travel_time = inturn[i+1][0] - inturn[i][-1]

    # 宿泊料金の合計が新幹線往復より安ければ
    if (travel_time * b) < (a * 2)
        total += travel_time * b # 宿泊する
...
### [C100:選曲の方法](https://paiza.jp/works/challenges/493/page/result)
...
count = 0 # 曲数
music_size = 0 # 現在の曲の長さ
music = [] # 曲目を入れる配列を定義

# 曲目に秒に変換してから格納
(0...n).each do |i|
    minutes , second = gets.split(" ").map(&:to_i)
    music << (minutes * 60) + second
end
# 曲目を昇順に並び替え
music = music.sort
...
### [C071:直角三角形](https://paiza.jp/works/challenges/340/page/result)
...
# 斜辺の最大値
limit_shahen = (n**2 + m**2)**(1/2.0)

# 条件に当てはまるものの数
count = 0

# 底辺iと高さjの組み合わせで条件に当てはまるか判定
(1...n).each do |i|
    (1...m).each do |j|
        # この組み合わせでの斜辺を求める
        shahen = (i ** 2 + j ** 2) ** (1/2.0)

        # 条件の最大値より小さくかつ整数であるかを判定
        if limit_shahen > shahen && (shahen % 1 == 0 )
            count += 1
        end
    end
end
